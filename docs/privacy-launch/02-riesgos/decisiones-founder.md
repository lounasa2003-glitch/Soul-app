...
- **Estado real de implementación — cierre de `cita_mensajes`/`citas`/`reportes` (2026-07-29)**: Implementado en código, **pendiente de migración, despliegue y prueba**. Corresponde a la propuesta técnica evaluada por separado en `propuesta-retencion-citas.md` y a la decisión aprobada sobre ese documento. Cambios de código (sin desplegar todavía):
  ...
  - Migración creada, **no ejecutada todavía**: `migracion_retencion_citas.sql` (agrega `citas.cerrada_en` y `reportes.estado`).
  - No se tocó `matches`, `historial_relacional` ni `api/calcularMatches.js` -- confirmado que ninguno de los cambios los afecta.
  - Sin desplegar ni probar en producción todavía: falta correr la migración en Supabase, hacer push, y verificar en vivo (cierre de una cita real, purga con una cita vencida de prueba, cambio de estado de un reporte desde el panel).
---
