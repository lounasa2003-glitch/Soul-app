...
- **Estado real de investigación (2026-07-28)**: Actualizada con documentación oficial vigente de los tres proveedores -- ver detalle completo y citas en `ficha-proveedores-transferencias.md`. Resumen de lo que quedó **confirmado con evidencia oficial** (no inferido): (1) Anthropic -- Soul usa la API comercial, no una cuenta de consumidor (confirmado por diseño técnico: una API key de `api.anthropic.com` no puede emitirse desde una cuenta Claude.ai Free/Pro/Max); los datos **no** se usan para entrenar modelos por defecto; retención estándar de **30 días** sin acuerdo especial; los modelos que usa Soul (`claude-sonnet-4-6`, `claude-haiku-4-5-20251001`) no son "Covered Models" (esa categoría, con retención obligatoria distinta, es solo Fable 5/Mythos 5). (2) Supabase -- corre sobre infraestructura AWS, con Google también como subprocesador. (3) Resend -- confirmado con cita textual que los metadatos/logs de la cuenta se almacenan en EE.UU. **sin importar qué región de envío tenga configurado el dominio** `soulapp.love`; lista completa de subprocesadores (todos en EE.UU.) confirmada; los datos concretos que Soul le envía (email, nombre, contenido de la notificación -- nunca el contenido real de una conversación) están verificados en el código. Queda **PENDIENTE DE CONFIRMACIÓN** (requiere entrar a cada dashboard, no verificable desde este entorno): si Soul tiene un acuerdo de Zero Data Retention con Anthropic (no hay evidencia de que exista, no se afirma que exista); la región exacta del proyecto de Supabase y si tiene réplicas en otras regiones; la región de envío configurada para el dominio de Resend. No alcanza todavía para completar Data Safety de Play Console con precisión total -- faltan esos tres datos puntuales de dashboard (~5 minutos cada uno). No se modificó código ni configuración de ninguna cuenta.
