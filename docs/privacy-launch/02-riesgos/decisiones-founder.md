...
- **Estado real de implementación**: Implementado, desplegado y verificado en producción (2026-07-28). Tabla nueva `solicitudes_revision_perfil` (`usuario_id`, `texto_cuestionado`, `explicacion`, `estado` [`pendiente`/`en_revision`/`resuelto`], `respuesta_admin`, `respondido_en`, `created_at`), con RLS "propio" (migración `migracion_solicitudes_revision_perfil.sql`). En "Mi perfil" (`soul.html`) se agregó el link "No estoy de acuerdo con una parte de mi perfil", que despliega un formulario corto (qué parte cuestiona / por qué) y lo guarda vía el mismo `guardarTabla()` genérico que ya usan `feedback_piloto`/`reportes_tecnicos` -- se sumó la tabla nueva a `TABLAS_PERMITIDAS` (`lib/authUtil.js`) y a `TABLAS_CON_RLS` (`api/guardar.js`, `api/leer.js`) para que el `INSERT` viaje con el token propio de la persona. En el panel admin, `api/admin/personas.js` suma `modo=solicitudesRevision` (listado, con nombre/email de quien pidió) y `modo=solicitudRevision` en `PATCH` (cambiar estado y guardar una respuesta, con fecha); `panel-admin.html` suma una sección nueva "Solicitudes de revisión de perfil". En ningún momento se toca `perfiles` -- la corrección real del perfil, si corresponde, la sigue haciendo la administradora a mano desde la Hoja de Vida ya existente, tal como pide la decisión (nada se reconstruye solo). Probado en producción con una cuenta de prueba real: el pedido se guardó correctamente (verificado leyendo la fila vía `/api/leer` con el token propio de la cuenta), y la founder confirmó directamente en el panel admin que el pedido aparece y que cambiar el estado y escribir una respuesta se guarda.Decisión final: Opción A implementada, desplegada y verificada en producción. La persona puede cuestionar una inferencia desde “Mi perfil”, la solicitud queda registrada y la administradora puede revisarla, cambiar su estado y responder sin modificar automáticamente el perfil.
...
Decisión final: Aprobación parcial. Se mantienen los plazos ya implementados: borrado a los 30 días de la eliminación de la cuenta para perfiles, conversaciones, historial relacional, intentos de fuga de prompt, reflexiones, ayudas y solicitudes de revisión; anonimización del feedback; 180 días para reportes técnicos y 90 días para errores silenciosos y diagnósticos diarios.

- **Estado real de implementación — cierre de `cita_mensajes`/`citas`/`reportes` (2026-07-29)**: Implementado, **migrado, desplegado y verificado en producción**. Corresponde a la propuesta técnica evaluada por separado en `propuesta-retencion-citas.md` y a la decisión aprobada sobre ese documento.
  - Corregida una fuga real encontrada durante esta ronda: varias pantallas y endpoints (Mis citas, Mis matches, Sala de Encuentros, debriefing, y tres vistas del panel admin) mostraban el email tombstone `borrada-{id}@soul-app.eliminado` como si fuera el nombre de la persona, apenas se anonimizaba una cuenta eliminada. Se agregó un helper compartido `nombreMostrable()` (`lib/authUtil.js`) que ahora se usa en todos esos puntos (`api/citas.js`, `api/matches.js`, `api/admin/personas.js`, `api/admin/comparar.js`, `lib/email.js`) -- muestra "Cuenta eliminada" en vez del email tombstone.
  - Columna nueva `citas.cerrada_en` (timestamptz), escrita desde `finalizarCita()` (`lib/cierreCita.js`) con un PATCH condicional (`cerrada_en=is.null`) para no pisar la fecha si ya existe.
  - Columna nueva `reportes.estado` (`abierto`/`cerrado`, default `abierto`), con un `PATCH` nuevo en el panel admin (`api/admin/personas.js`, `modo=reporte`) y un control para cambiarlo desde la Hoja de Vida → Seguridad (`panel-admin.html`).
  - Purga nueva `purgarCitaMensajesVencidos()` (`api/cron/diagnostico-diario.js`), corrida en cada ejecución diaria del cron: borra `cita_mensajes` de citas cerradas hace más de 12 meses (`cerrada_en`) y limpia los campos derivados por IA en `citas` (`resumen_ia`, `insights_debriefing_a/b`, `perfil_cita_a/b`, `compatibilidad_cita_a/b`, `refinamiento_a/b`) -- nunca borra la fila de `citas`. Si existe un reporte con `estado='abierto'` sobre el `match_id` de la cita, se frena la purga de todas las citas de ese match.
  - No se tocó `matches`, `historial_relacional` ni `api/calcularMatches.js` -- confirmado que ninguno de los cambios los afecta.
  - **Migración**: `migracion_retencion_citas.sql` corrida en Supabase el 2026-07-29 (`citas.cerrada_en`, `reportes.estado` con default `abierto` y CHECK `abierto`/`cerrado`) -- confirmado leyendo el esquema real vía PostgREST antes y después de correrla.
  - **Despliegue**: commit `4ab5083` pusheado a `main`. El dominio `soulapp.love` quedó inalcanzable durante la verificación (problema de dominio, no del build -- el deploy del commit compiló bien); las pruebas de abajo se corrieron contra la URL de deployment de Vercel (`soul-app-git-main-lounasa2003-glitchs-projects.vercel.app`), que sirve el mismo código. Pendiente confirmar que `soulapp.love` volvió a responder.
  - **Pruebas en producción (2026-07-29)**, con dos cuentas descartables creadas para este fin (`contacto+retencionA.../contacto+retencionB...@soulapp.love`, match armado vía el comparador admin, sin alterar cuentas reales ni fechas de citas reales -- ambas cuentas quedaron marcadas para borrado al terminar):
    - `cerrada_en` se confirmó seteado (`2026-07-29T16:57:38Z`) al cerrar una cita real vía la API (`ayudaPrivada` tipo `cerrar` + `responderCierre`).
    - `reportes.estado` se confirmó pasando de `abierto` a `cerrado` (y viceversa) desde el `PATCH` nuevo del panel admin, reflejado correctamente en la Hoja de Vida.
    - El tombstone se confirmó resuelto en dos puntos reales: "Mis citas" (`otra_persona_nombre`) y la Hoja de Vida del panel admin, ambos mostrando "Cuenta eliminada" en vez del email tombstone, tras marcar la cuenta de prueba como `cuenta_eliminada`.
    - La lógica de "reporte abierto frena la purga" se confirmó corriendo la misma consulta que usa `purgarCitaMensajesVencidos()` (`reportes` por `match_id` con `estado=eq.abierto`) contra el match de prueba: aparece con el reporte abierto, desaparece al cerrarlo -- sin ejecutar la purga real ni forzar una purga de 12 meses ni alterar fechas reales, tal como se pidió.Decisión final: Opción A implementada, migrada, desplegada y verificada. Soul aplica plazos diferenciados de retención, anonimiza las cuentas eliminadas, conserva las citas sin transcripciones íntimas después del plazo definido y suspende la purga cuando existe un reporte abierto.
---
...
- **Estado real de implementación (2026-07-29)**: Implementado, migrado, desplegado y verificado en producción. Se confirmó por búsqueda exhaustiva en todo el repositorio que `hora_nacimiento` no se usa en ningún prompt de IA, en el matching (`lib/matchCompatible.js`), en la compatibilidad (`lib/comparePrompt.js`), en ningún filtro, en el panel admin (`panel-admin.html`/`api/admin/*.js`) ni en reportes -- tampoco estaba nunca en `CAMPOS_BASICOS_REQUERIDOS` (`api/guardar.js`) ni en `CAMPOS_FORMULARIO` (`soul.html`), es decir, nunca fue obligatoria ni contaba para el porcentaje de perfil completo. Se eliminaron los 5 puntos donde el código la pedía/guardaba/mostraba: el campo del formulario de onboarding (Etapa 1), el campo de "Mi perfil", y los tres puntos de JS que la enviaban o cargaban (`guardarUsuarioYContinuar`, `mostrarMiPerfil`, `guardarMiPerfil`) en `soul.html`. La columna en Supabase **no se borra** -- se agregó `migracion_hora_nacimiento_obsoleta.sql` (`COMMENT ON COLUMN usuarios.hora_nacimiento IS 'OBSOLETA...'`), que solo documenta la columna como obsoleta en el esquema sin tocar ningún valor. El único lugar del código que todavía menciona `hora_nacimiento` es la anonimización al eliminar una cuenta (`api/cron/diagnostico-diario.js`), que sigue poniéndola en `null` junto con el resto de los datos personales -- eso es borrado, no una escritura nueva, así que se dejó igual a propósito.
  - **Código**: ya estaba commiteado y pusheado a `main` desde antes de esta ronda (commit `4cb394a`), como parte de la base sobre la que se hicieron los despliegues posteriores de la Decisión 6 -- no hubo cambios de código nuevos que commitear en esta ronda.
  - **Migración**: `migracion_hora_nacimiento_obsoleta.sql` corrida en Supabase el 2026-07-29 (solo agrega un `COMMENT ON COLUMN`, no cambia ningún dato ni columna -- no verificable vía la API REST con la anon key, se tomó la confirmación directa de la founder de haberla ejecutado sin error).
  - **Pruebas en producción (2026-07-29)**, con una cuenta descartable creada para este fin (marcada para borrado al terminar): se completó el onboarding real de punta a punta (los 8 pasos + banco cultural) contra el deployment de Vercel -- se confirmó por inspección del DOM completo que no existe ningún campo de hora de nacimiento ni en el onboarding ni en "Mi perfil" (solo "Cuándo naciste", la fecha); se confirmó que el intake se completó sin error y `etapa_actual` avanzó correctamente a `chat` (Soul saludó a la cuenta de prueba), o sea que el resto del flujo sigue funcionando igual que antes; y se confirmó, leyendo la fila real de la cuenta de prueba vía el panel admin después de completar el onboarding, que `hora_nacimiento` quedó en `null` mientras que `fecha_nacimiento` sí se guardó -- ningún valor nuevo se escribe en esa columna.
