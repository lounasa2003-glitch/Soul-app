...
|---|---|---|---|---|---|
| P-01 | Verificación de edad mínima (18+) en cliente | CUMPLE | `soul.html:702,2293-2300,3919` (tope `max` en el date picker + bloqueo de avance de paso) | Ninguna | — |
| P-02 | Verificación de edad mínima (18+) en servidor, en el onboarding | CUMPLE | `api/guardar.js:117-130` (`EDAD_MINIMA`, bloquea `etapa_actual='chat'` si `edad < EDAD_MINIMA`) | Ninguna | — |
| P-03 | Verificación de edad mínima al **editar** la fecha de nacimiento después del onboarding | NO CUMPLE | `soul.html:3268-3299` (`guardarMiPerfil`) no envía `etapa_actual`, por lo que el chequeo de `api/guardar.js:122` no se ejecuta en esa ruta | Aplicar el mismo chequeo de edad al PATCH de "Mi perfil" cuando cambia `fecha_nacimiento` | Desarrollo |
| P-04 | Declaración jurada explícita y separada de mayoría de edad (más allá de la fecha) | NO CONFIRMADO | `docs/privacy-launch/02-riesgos/decisiones-founder.md` Decisión 1: "La declaración explícita separada... queda pendiente de decidir" | Decidir si se agrega una casilla separada, además de la fecha de nacimiento | Producto/Legal |
...
| P-15 | Política de menores: la app declara y valida que es exclusivamente para mayores de 18 | PARCIAL | Ver P-01 a P-04 | Cerrar P-03 y P-04 | Desarrollo + Producto |
...
| O-10 | Declaración de mayoría de edad dentro del consentimiento | CUMPLE | `soul.html:702` | Ver P-04 sobre si además se quiere una casilla separada | Producto/Legal |
...
|---|---|---|---|---|
| G-01 | Restricción para menores (declaración + control técnico) | PARCIAL | Ver P-01 a P-04 | Cerrar P-03/P-04 |
