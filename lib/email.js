...
import { nombreMostrable } from './authUtil.js';
...
// Comunicación DE PRODUCTO (aviso de match) -- distinta de las esenciales
// (confirmación de cuenta, recuperación de acceso), que nunca se gatean acá.
...
// Cada persona del match puede tener su propia preferencia -- 'otro' se
// resuelve siempre contra la lista COMPLETA (todos), no la ya filtrada por
// consentimiento, para que su nombre siga apareciendo en el aviso de quien
// sí quiere recibirlo, aunque la otra parte haya optado por no recibir nada.
export async function notificarNuevoMatch(destinatarios) {
  const todos = (destinatarios || []).filter(d => d && d.email);
  const autorizados = todos.filter(d => d.comunicaciones_producto_aceptadas === true);
  await Promise.all(autorizados.map(d => {
    const otro = todos.find(x => x !== d);
    const propioLabel = nombreMostrable(d);
    const otroLabel = nombreMostrable(otro);
    return enviarEmail({
      to: d.email,
...
// quedó.
// DE PRODUCTO (recordatorio/reactivación) -- ver nota de gate en
// notificarNuevoMatch.
export async function notificarDatosIncompletos({ nombre, email, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
    to: email,
...
// DE PRODUCTO -- ver nota de gate en notificarNuevoMatch.
export async function notificarSalaEncuentrosPendiente({ nombre, email, remitenteNombre, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
...
// Recordatorio para quien tiene un match activo esperando su elección
// (aceptar/rechazar) y todavía no contestó nada -- ver lib/recordatorioMatches.js
// para el criterio de cuándo se dispara y el guard que evita mandarlo dos veces.
// DE PRODUCTO -- ver nota de gate en notificarNuevoMatch.
export async function notificarMatchSinDecision({ nombre, email, otroNombre, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
    to: email,
    subject: 'Alguien te está esperando en Soul',
    html: `<p>Hola ${propioLabel},</p><p>Tenés un match esperando tu elección${otroNombre ? (': <strong>' + otroNombre + '</strong>') : ''}. Cuando quieras, entrá a decidir si seguís -- no hay apuro, sigue ahí.</p><p><a href="${APP_URL}/soul.html">Entrar a Soul</a></p><p style="color:#888;font-size:.8rem;margin-top:1rem">Aviso para la cuenta de ${propioLabel}.</p>`
  });
}

// Recordatorio para quien aceptó un match (ya existe la cita/encuentro) pero
// todavía no escribió ningún mensaje ahí -- distinto del caso anterior,
// donde ni siquiera eligió. Ver lib/recordatorioMatches.js.
// DE PRODUCTO -- ver nota de gate en notificarNuevoMatch.
export async function notificarCitaSinMensaje({ nombre, email, otroNombre, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
    to: email,
    subject: 'Tu encuentro en Soul sigue esperando tu primer mensaje',
    html: `<p>Hola ${propioLabel},</p><p>Elegiste seguir adelante${otroNombre ? (' con <strong>' + otroNombre + '</strong>') : ''}, pero todavía no escribiste nada en esa charla. Cuando quieras, entrá y segui -- sin apuro.</p><p><a href="${APP_URL}/soul.html">Entrar a Soul</a></p><p style="color:#888;font-size:.8rem;margin-top:1rem">Aviso para la cuenta de ${propioLabel}.</p>`
  });
}

// Recordatorio para quien tuvo un encuentro que ya cerró (manual o por
// inactividad) y todavía no abrió el debriefing corto -- distinto de los
// dos de arriba: acá la persona ya llegó a tener el encuentro, solo falta
// que vuelva a cerrar el círculo. Ver lib/recordatorioMatches.js.
// DE PRODUCTO -- ver nota de gate en notificarNuevoMatch.
export async function notificarDebriefingPendiente({ nombre, email, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
    to: email,
    subject: 'Soul preparó una reflexión sobre tu encuentro',
    html: `<p>Hola ${propioLabel},</p><p>Tu último encuentro ya cerró y Soul tiene una reflexión lista para vos sobre cómo te fue. Cuando quieras, entrá a verla -- no hace falta que sea hoy.</p><p><a href="${APP_URL}/soul.html">Entrar a Soul</a></p><p style="color:#888;font-size:.8rem;margin-top:1rem">Aviso para la cuenta de ${propioLabel}.</p>`
  });
}

// Recordatorio para quien terminó de hablar con Soul pero todavía no
// confirmó la representación interpretativa ("Esto me representa") --
// hasta que lo haga, no entra a las búsquedas de compatibilidad. Ver
// lib/recordatorioPerfilPendiente.js (24h y 72h, máximo dos avisos).
// DE PRODUCTO -- ver nota de gate en notificarNuevoMatch.
export async function notificarPerfilPendiente({ nombre, email, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
    to: email,
    subject: 'Tu perfil en Soul sigue pendiente de confirmar',
    html: `<p>Hola ${propioLabel},</p><p>Tu perfil todavía está pendiente de confirmar. Hasta que lo revises y aceptes, Soul no te incluirá en las búsquedas de compatibilidad.</p><p><a href="${APP_URL}/soul.html">Entrar a Soul</a></p><p style="color:#888;font-size:.8rem;margin-top:1rem">Aviso para la cuenta de ${propioLabel}.</p>`
  });
}

// DE PRODUCTO -- ver nota de gate en notificarNuevoMatch.
export async function notificarMensajeCita({ nombre, email, remitenteNombre, comunicaciones_producto_aceptadas }) {
  if (!email) return false;
  if (comunicaciones_producto_aceptadas !== true) return false;
  const propioLabel = nombre || email;
  return await enviarEmail({
