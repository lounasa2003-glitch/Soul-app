...
import { chequearRecordatoriosPerfilPendiente } from './recordatorioPerfilPendiente.js';
import { vencimientoEsFuturo } from './suscripcionesMercadoPago.js';

// Apagado (Decision: fin del piloto con todos en Pro gratis) -- de acá en
// más cada cuenta ve su plan real, tal cual está guardado en
// usuarios.plan (default 'free', el panel admin sigue siendo la forma de
// pasar a alguien a 'pro' a mano). Efecto inmediato para TODAS las cuentas
// existentes, no solo las nuevas -- quien ya tenía acceso Pro gratuito
// durante el piloto lo pierde salvo que su fila ya diga plan='pro'. Volver
// a poner esto en `true` si hace falta reabrir Pro gratis para todo el
// mundo otra vez.
const TODOS_PRO_TEMPORAL = false;

// Excepcion puntual a TODOS_PRO_TEMPORAL para poder probar el producto real
// en Free mientras el resto del piloto sigue viendo Pro sin cambios. Un solo
// email, configurado por variable de entorno (nunca hardcodeado, nunca
// mandado por el cliente) -- si no esta seteada o queda vacia despues de
// normalizar, no se aplica ninguna excepcion y el comportamiento es
// identico al de antes de este cambio. Nunca se loguea ni el email ni el
// valor de esta variable (ver comparacionEsCuentaPruebaFree mas abajo).
const CUENTA_PRUEBA_FREE_EMAIL_NORMALIZADO = normalizarEmail(process.env.CUENTA_PRUEBA_FREE_EMAIL);

// trim()+toLowerCase() de los dos lados de la comparacion -- Supabase Auth
// normaliza el email a minusculas al autenticar, pero la variable de
// entorno la tipea una persona a mano, asi que puede traer mayusculas o
// espacios de mas sin que sea un error real. Coincidencia EXACTA solamente
// -- nunca includes/startsWith/regex, para que una cuenta con un email
// parecido (ej. mismo nombre en otro dominio) nunca caiga en la excepcion
// por accidente.
...
function esCuentaPruebaFree(emailAutenticado) {
  if (!CUENTA_PRUEBA_FREE_EMAIL_NORMALIZADO) return false;
  return normalizarEmail(emailAutenticado) === CUENTA_PRUEBA_FREE_EMAIL_NORMALIZADO;
}
...
export async function verificarUsuario(req) {
  ...
  const rowRes = await fetch(
    `${supabaseUrl}/rest/v1/usuarios?select=id,analisis_usados,ultima_actividad,mail_confirmado,etapa_actual,plan,plan_origen,mp_status,plan_auto_renueva,plan_vencimiento,eliminacion_solicitada_en&email=eq.${encodeURIComponent(authUser.email)}`,
    { headers: { apikey: supabaseKey, Authorization: `Bearer ${token}` } }
  );
  ...
  // Gracia de cancelacion de Mercado Pago (ver camposDesdeMP en
  // lib/suscripcionesMercadoPago.js): una suscripcion cancelada conserva
  // plan='pro' hasta plan_vencimiento. Nadie mas vigila esa fecha por su
  // cuenta -- no hay cron para esto, es deliberado -- asi que se resuelve
  // aca, en el gate que ya corre en cada request autenticado. Solo actua
  // sobre plan_origen='mercadopago' (nunca un Pro de Google Play ni un
  // manual/cortesia) y solo mientras plan siga en 'pro' -- una vez que pisa
  // a 'free', esta misma condicion deja de dispararse por si sola
  // (idempotente por construccion, sin necesitar una tabla de estado aparte).
  if (fila && fila.plan === 'pro' && fila.plan_origen === 'mercadopago' &&
      fila.mp_status === 'cancelled' && fila.plan_auto_renueva === false &&
      !vencimientoEsFuturo(fila.plan_vencimiento)) {
    // Con la service role key, nunca con el token de la persona: este PATCH
    // lleva ademas los filtros de plan/plan_origen/mp_status/plan_auto_renueva
    // en la URL (no solo el id) para que sea una actualizacion condicional
    // atomica -- si alguien reactivo la suscripcion justo en el medio
    // (webhook, sync, otra pestaña), esos filtros ya no matchean nada y
    // Supabase devuelve 0 filas en vez de pisar un estado nuevo por encima.
    const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceKey) {
      throw new Error('Falta SUPABASE_SERVICE_ROLE_KEY (verificarUsuario: gracia de cancelacion)');
    }
    const patchRes = await fetch(
      `${supabaseUrl}/rest/v1/usuarios?id=eq.${encodeURIComponent(fila.id)}&plan=eq.pro&plan_origen=eq.mercadopago&mp_status=eq.cancelled&plan_auto_renueva=eq.false`,
      {
        method: 'PATCH',
        headers: {
          apikey: serviceKey,
        ...
        body: JSON.stringify({ plan: 'free', plan_vencimiento: null, plan_actualizado_en: new Date().toISOString() })
      }
    );
    if (!patchRes.ok) {
      throw new Error(`No se pudo actualizar usuarios: HTTP ${patchRes.status}`);
    }
    const filasActualizadas = await patchRes.json();
    // Cero filas significa que la condicion ya no matcheaba al momento de
    // escribir (alguien reactivo la suscripcion concurrentemente) -- NO se
    // asume 'free' en ese caso, se deja 'fila' tal cual esta y la proxima
    // peticion autenticada la vuelve a consultar de cero.
    if (filasActualizadas.length > 0) {
      fila.plan = 'free';
      fila.plan_vencimiento = null;
    }
  }
  ...
  // Mismo criterio: recordatorio para quien terminó de hablar con Soul pero
  // todavía no confirmó su representación interpretativa. Ver
  // lib/recordatorioPerfilPendiente.js.
  chequearRecordatoriosPerfilPendiente(supabaseUrl, supabaseKey).catch(() => {});
  ...
  return {
    ...
    // pisa a 'pro' aca mismo para todo el mundo, EXCEPTO la cuenta de prueba
    // (CUENTA_PRUEBA_FREE_EMAIL, comparada de forma normalizada y exacta
    // via esCuentaPruebaFree), a la que se le respeta el plan real de la
    // base para poder probar el producto tal cual lo ve alguien Free.
    plan: (TODOS_PRO_TEMPORAL && !esCuentaPruebaFree(authUser.email)) ? 'pro' : (fila ? (fila.plan || 'free') : 'free')
  };
}
