...
export async function verificarUsuario(req) {
  ...
  const rowRes = await fetch(
    `${supabaseUrl}/rest/v1/usuarios?select=id,analisis_usados&email=eq.${encodeURIComponent(authUser.email)}`,
    { headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` } }
  );
  const rows = rowRes.ok ? await rowRes.json() : [];
  return {
    email: authUser.email,
    usuarioId: rows[0] ? rows[0].id : null,
    analisisUsados: rows[0] ? rows[0].analisis_usados : 0
  };
}
