...
export async function chequearRecordatoriosIntake(supabaseUrl, supabaseKey) {
  // 'usuarios' ya tiene politica RLS real -- este chequeo recorre TODAS las
  // personas del piloto buscando quien dejo la Etapa 1 a medias, no una sola
  // sesion, asi que no hay ningun token propio que tenga sentido usar aca.
  // Service role key, mismo criterio que lib/recordatorioMatches.js.
  ...
  try {
    ...
    const res = await fetch(
      `${supabaseUrl}/rest/v1/usuarios?select=id,nombre,email,comunicaciones_producto_aceptadas&etapa_actual=eq.nuevo&recordatorio_datos_enviado_en=is.null&created_at=lt.${encodeURIComponent(limite)}`,
      { headers }
    );
    const pendientes = res.ok ? await res.json() : [];
    for (const u of pendientes) {
      if (!u.email) continue;
      enviarPushAUsuario(u.id, {
        titulo: 'Te está esperando Soul',
        cuerpo: 'Dejaste algunas preguntas sin terminar -- retomá cuando quieras.',
        data: { tipo: 'recordatorio_intake' }
      }).catch(() => {});
      const enviado = await notificarDatosIncompletos({ nombre: u.nombre, email: u.email, comunicaciones_producto_aceptadas: u.comunicaciones_producto_aceptadas });
      if (enviado) {
