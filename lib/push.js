...
// sus propios datos.
//
// `esencial` distingue avisos indispensables (seguridad/funcionamiento de
// la cuenta) de avisos DE PRODUCTO (match nuevo, recordatorios, mensaje de
// cita, etc.). Por defecto es `false` -- es decir, todo push se trata como
// DE PRODUCTO salvo que quien llama declare explicitamente lo contrario --
// para que un llamado nuevo no quede exento de este control por omision.
// Si es DE PRODUCTO, se respeta la misma preferencia que ya gatea los
// emails de producto (comunicaciones_producto_aceptadas, ver lib/email.js):
// false/null/undefined se tratan igual, como no autorizado.
export async function enviarPushAUsuario(usuarioId, { titulo, cuerpo, data, esencial = false }) {
  ...
  if (!esencial) {
    const prefRes = await fetch(`${supabaseUrl}/rest/v1/usuarios?select=comunicaciones_producto_aceptadas&id=eq.${encodeURIComponent(usuarioId)}`, { headers });
    const prefFila = (prefRes.ok ? await prefRes.json() : [])[0];
    if (!prefFila || prefFila.comunicaciones_producto_aceptadas !== true) {
      return { enviados: 0, resultados: [], bloqueadoPorPreferencia: true };
    }
  }
