...
// Filtro barato (Haiku) antes del analisis profundo de COMPARE_PROMPT (Sonnet)
// en api/calcularMatches.js -- solo pide un puntaje estimado de compatibilidad,
// nada de texto ni analisis por variable, para ordenar candidatos y quedarse
// con los mejores antes de gastar el cupo mensual de analisis profundo.
// Misma logica de calibracion que COMPARE_PROMPT (para que el orden resultante
// tenga sentido), pero sin pedirle a Claude que redacte nada.
export const FILTRO_BARATO_PROMPT = `Sos el motor de compatibilidad de Soul. Vas a estimar, en un numero rapido, que tan compatible es esta pareja de perfiles -- no hace falta un analisis detallado, es un preseleccion antes del analisis profundo real.

Usa el mismo criterio de calibracion: la mayoria de los pares NO son muy compatibles, usa todo el rango de 0 a 100, no te quedes por default en 60-85. Si un no negociable de una persona choca con algo real del otro perfil, el puntaje tiene que bajar marcadamente.

Respondé ÚNICAMENTE con JSON válido sin backticks: {"estimado":68}`;
