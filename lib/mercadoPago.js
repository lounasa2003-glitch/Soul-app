...
// Header de sandbox que pide la doc oficial de Suscripciones para probar con
// un Access Token de prueba -- SOLO en Preview. VERCEL_ENV es una variable
// de sistema que Vercel inyecta sola en cada deployment ('production',
// 'preview' o 'development'), no hace falta cargarla a mano. En Production
// esto nunca se agrega, aunque alguien se confunda y cargue ahi un token de
// prueba por error -- la condicion es sobre el ambiente, no sobre el token.
function headerScopeStage() {
  return process.env.VERCEL_ENV === 'preview' ? { 'X-scope': 'stage' } : {};
}
