...
// pasa solo server-side, ver lib/suscripcionesMercadoPago.js). Production
// conserva el dominio publico. En Preview, Vercel inyecta VERCEL_URL y se
// vuelve al mismo deployment que inicio el checkout, donde pro-retorno.html
// si existe. No hace falta agregar ni cambiar variables manualmente.
const BACK_URL = 'https://soulapp.love/pro-retorno.html';

function resolverBackUrl() {
  if (process.env.VERCEL_ENV === 'preview' && process.env.VERCEL_URL) {
    return `https://${process.env.VERCEL_URL}/pro-retorno.html`;
  }
  return BACK_URL;
}
...
export async function crearPreapproval({ usuarioId, email, montoARS }) {
  ...
  // Solo en Preview/sandbox: si esta cargada, usa el email de la cuenta de
  // prueba "Comprador" de Mercado Pago en vez del email real de la persona
  // -- evita que un checkout de prueba quede asociado a una cuenta real de
  // MP (ver conversacion sobre el problema del sandbox). En Production esta
  // env var no existe, asi que payerEmailFinal cae siempre al email real,
  // sin cambio de comportamiento. external_reference sigue siendo SIEMPRE
  // el usuarioId real de Soul -- es lo unico de lo que depende el
  // entitlement (ver lib/suscripcionesMercadoPago.js), independiente de con
  // que email se haya logueado en Mercado Pago.
  const payerEmailFinal = process.env.MERCADOPAGO_PAYER_EMAIL_PRUEBA || email;

  // DIAGNOSTICO TEMPORAL -- por que MP rechaza payer_email como invalido en
  // Preview pese a haber recargado MERCADOPAGO_PAYER_EMAIL_PRUEBA. Nunca
  // registra el email completo, solo su forma (mismo criterio que el
  // diagnostico de SOUL_PRO_PRECIO_USD). Sacar apenas se resuelva.
  const diagnosticoPayerEmail = {
    payerEmailSource: process.env.MERCADOPAGO_PAYER_EMAIL_PRUEBA ? 'env_prueba' : 'usuario',
    payerEmailLength: payerEmailFinal.length,
    payerEmailHasWhitespace: /\s/.test(payerEmailFinal),
    payerEmailEndsWithTestuser: payerEmailFinal.endsWith('@testuser.com'),
    payerEmailFormatValid: /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payerEmailFinal)
  };
  ...
  const { ok, status, data } = await llamarMP('/preapproval', {
    ...
    body: JSON.stringify({
      ...
      back_url: resolverBackUrl(),
      status: 'pending'
    })
  });

  if (!ok || !data || !data.id || !data.init_point) {
    throw new Error(`No se pudo crear la suscripcion en Mercado Pago: HTTP ${status} ${JSON.stringify(data)} [diagnostico payer_email: ${JSON.stringify(diagnosticoPayerEmail)}]`);
  }
...
export async function obtenerPreapproval(preapprovalId) {
  ...
  if (!ok) {
    const error = new Error(`Error consultando preapproval en Mercado Pago: HTTP ${status} ${JSON.stringify(data)}`);
    error.statusMercadoPago = status;
    const codigoCrudo = data && (data.code || data.error);
    error.codigoMercadoPago = codigoCrudo
      ? String(codigoCrudo).replace(/[^A-Za-z0-9_.-]/g, '').slice(0, 80)
      : null;
    throw error;
  }
  return data;
