...
export async function chequearLimite(email, endpoint, limite, ventanaSegundos) {
  ...
  // rate_limits es de acceso exclusivo server-side (sin RLS/policy para
  // anon ni authenticated, ver migracion_rls_tablas_internas.sql) -- el
  // anon key ya no alcanza aca, hace falta la service role key.
