...
// una busca para el vinculo en si -- la compatibilidad es que haya
// coincidencia entre lo que busca cada una, no que una acepte el atributo
// de la otra. "Todavía no lo sé" no restringe (esta abierta) porque
// todavia no decidio; el resto exige que se solapen -- cruzar "Romántico"
// (nada mas) con "Amistad profunda" (nada mas) es exactamente el tipo de
// match que no tiene sentido generar.
//
// OJO: el formulario (ver selChipM/selChipMPerfil en soul.html) permite
// elegir MAS DE UNA opcion aca -- tipo_vinculo se guarda como ARRAY en
// 'usuarios' (ej. ["Romántico","Compañía"]), nunca como texto suelto. Una
// version anterior de esta funcion comparaba con === asumiendo texto
// simple, lo que en la practica bloqueaba a TODO el mundo (un array nunca
// es === a otro array, ni === al string "Todavía no lo sé", aunque
// contenga exactamente eso) -- se detecto porque el ranking manual daba
// "no hay perfiles" para una cuenta real con datos completos.
const TIPO_VINCULO_ABIERTO = 'Todavía no lo sé';
function normalizarTipoVinculo(valor) {
  if (Array.isArray(valor)) return valor;
  if (typeof valor === 'string' && valor) return [valor]; // por si alguna fila vieja lo tiene como texto suelto
  return [];
}
export function tipoVinculoCompatible(personaA, personaB) {
  const a = normalizarTipoVinculo(personaA && personaA.tipo_vinculo);
  const b = normalizarTipoVinculo(personaB && personaB.tipo_vinculo);
  if (a.length === 0 || b.length === 0) return false;
  if (a.includes(TIPO_VINCULO_ABIERTO) || b.includes(TIPO_VINCULO_ABIERTO)) return true;
