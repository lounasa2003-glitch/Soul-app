import { notificarMatchSinDecision, notificarCitaSinMensaje } from './email.js';
...
async function avisarEleccionPendiente(supabaseUrl, headers, limite) {
  for (const lado of ['a', 'b']) {
    ...
    const usuariosRes = await fetch(`${supabaseUrl}/rest/v1/usuarios?select=id,nombre,email&id=in.(${idsUsuarios.map(encodeURIComponent).join(',')})`, { headers });
    ...
    for (const m of pendientes) {
      ...
      const enviado = await notificarMatchSinDecision({ nombre: propio.nombre, email: propio.email, otroNombre: otro ? otro.nombre : null });
      if (enviado) {
...
async function avisarCitaSinMensaje(supabaseUrl, headers, limite) {
  for (const lado of ['a', 'b']) {
    ...
    for (const cand of candidatas) {
      ...
      if (!propio || !propio.email) continue;
      const enviado = await notificarCitaSinMensaje({ nombre: propio.nombre, email: propio.email, otroNombre: otro ? otro.nombre : null });
      if (enviado) {
