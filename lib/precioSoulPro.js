...
function precioObjetivoUSD() {
  ...
  if (!crudo || !Number.isFinite(numero) || numero <= 0) {
    // DIAGNOSTICO TEMPORAL -- sacar apenas se resuelva por que el Preview de
    // Mercado Pago no esta viendo SOUL_PRO_PRECIO_USD pese a estar cargada.
    // Nunca registra el valor crudo (podria tener espacios/formato raro que
    // igual no hace falta ver caracter por caracter) -- solo su forma, via
    // el mensaje del Error que ya termina en errores_silenciosos
    // (registrarErrorSilencioso en cada catch que llama a esta funcion).
    const diagnostico = `existe=${crudo !== undefined} typeof=${typeof crudo} longitud=${typeof crudo === 'string' ? crudo.length : 'null'} Number=${numero}`;
    throw new Error(`Falta o es invalido SOUL_PRO_PRECIO_USD [diagnostico: ${diagnostico}]`);
  }
  return numero;
}
