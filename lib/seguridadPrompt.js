...
export async function registrarIntentoFuga(usuario, mensaje, endpoint) {
  const usuarioId = usuario ? usuario.usuarioId : null;
  try {
    const supabaseUrl = process.env.SUPABASE_URL;
    const supabaseKey = process.env.SUPABASE_ANON_KEY;
    // 'intentos_fuga_prompt' ya tiene politica RLS real (ver
    // migracion_rls_intentos_fuga_prompt.sql) -- token propio.
    await fetch(`${supabaseUrl}/rest/v1/intentos_fuga_prompt`, {
      method: 'POST',
      headers: {
        apikey: supabaseKey,
        Authorization: `Bearer ${usuario ? usuario.token : supabaseKey}`,
    ...
    await registrarErrorSilencioso({ contexto: 'lib/seguridadPrompt: registrar intento de fuga', error: e, meta: { usuarioId, endpoint } });
  }
}
