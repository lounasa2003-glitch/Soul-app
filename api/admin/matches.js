...
async function cambiarEstado(req, res, supabaseUrl, headers, accion) {
  ...
  if (accion === 'activar' && data[0]) {
    ...
    try {
      const nRes = await fetch(`${supabaseUrl}/rest/v1/usuarios?select=nombre,email,cuenta_eliminada,comunicaciones_producto_aceptadas&id=in.(${encodeURIComponent(match.usuario_a)},${encodeURIComponent(match.usuario_b)})`, { headers });
